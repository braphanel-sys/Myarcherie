...
- `sw.js` — cache `archerAI-v4.6.7-dev` (branche dev) — V4.6.7-dev : auto-export ZIP debug en fin de session, garde-fou via bandeau version "dev", précache `app.js` ; install précharge `/`, `/index.html`, `/guide-scoring.html` ; fetch : cache-first pour les GET, bypass total pour `/api/` ; écoute le message `'skipWaiting'` envoyé par le bandeau de mise à jour.
